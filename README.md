...
- **Just browse**: Visit any page on `ocw.mit.edu`. The extension will silently track it and count your progress.
- **Jump back**: If you're on a different OCW page than your last session, the corner widget will slide in. Click it to return.
- **Check progress**: Click the extension icon to see your course progress bars with percentage completion.
...
It's built with vanilla JS, HTML, and CSS (Manifest V3). No frameworks, no build steps. The floating widget uses a Shadow DOM to keep styles completely isolated from the MIT OCW website, so it won't break or slow down the page. Course progress is detected by scanning the course navigation sidebar links to count total content pages.
